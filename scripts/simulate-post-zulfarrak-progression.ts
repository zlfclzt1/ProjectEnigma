...
import { buildPostZulfarrakProgressionBaseline } from "./post-zulfarrak-progression-simulation";

const projectRoot = fileURLToPath(new URL("../", import.meta.url));
const contentRoot = path.join(projectRoot, "content");
const fixturePath = path.join(
  projectRoot,
  "tests/fixtures/post-zulfarrak-progression-baseline.json",
);
...
const output = buildPostZulfarrakProgressionBaseline(loadContentRegistry(readContentModules()));
...
if (process.argv.includes("--write")) {
  ...
  assert.equal(
    ...
    serialized,
    "后祖尔法拉克进度基线已漂移；请核对后显式运行 npm run post-zulfarrak:baseline:write。",
  );
  console.log("后祖尔法拉克进度基线一致：当前内容上限与主动活动收益未漂移。");
} else {
