...
describe("dungeon content audit", () => {
  it("lists every route node and preserves optional/rare semantics", () => {
    ...
    expect(audit.routes).toHaveLength(215);
    expect(audit.routes.filter((row) => row.nodeType === "optional")).toHaveLength(35);
    expect(audit.routes.filter((row) => row.nodeType === "rare")).toHaveLength(34);
    expect(
      audit.routes.some(
