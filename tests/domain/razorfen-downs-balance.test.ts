...
import balance from "../fixtures/dungeon-balance/razorfen_downs.json";
import { loadBrowserContentRegistry } from "../../src/content/manifest";
import { asBrandedId } from "../../src/domain/shared/ids";

const content = loadBrowserContentRegistry();

describe("Razorfen Downs balance", () => {
  it("keeps the recommended-level required route in the agreed clear-rate band", () => {
    ...
    expect(balance.scenarios.standard.required.clearRate).toBeLessThanOrEqual(0.89);
  ...
  it("keeps the escort rewarding but noticeably harder for an underleveled party", () => {
    ...
    expect(balance.scenarios.overlevel["with-optional"].clearRate).toBeLessThan(0.72);
    expect(balance.scenarios.overlevel["with-optional"].clearRate).toBeLessThan(
      balance.scenarios.overlevel.required.clearRate,
    );
  });

  it("preserves high recovery pressure, rare-route cost, and the full-route time floor", () => {
