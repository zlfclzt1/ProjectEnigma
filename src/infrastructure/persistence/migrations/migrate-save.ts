...
export function migrateSave(
  ...
  if (persisted.saveVersion === 4) {
    return { state: migrateV4ToV5(persisted, content), migrated: true };
  }
  if (persisted.saveVersion === 3) {
    return { state: migrateV4ToV5(migrateV3ToV4(persisted), content), migrated: true };
  }
  if (persisted.saveVersion === 2) {
    return {
      state: migrateV4ToV5(migrateV3ToV4(migrateV2ToV3(persisted, content)), content),
      migrated: true,
    };
  }
