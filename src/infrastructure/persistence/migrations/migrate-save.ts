...
export function migrateSave(
  ...
  if (persisted.saveVersion === 6) {
    return { state: migrateV6ToV7(persisted, content), migrated: true };
  }
  if (persisted.saveVersion === 5) {
    return { state: migrateV6ToV7(migrateV5ToV6(persisted), content), migrated: true };
  }
  if (persisted.saveVersion === 4) {
    return {
      state: migrateV6ToV7(migrateV5ToV6(migrateV4ToV5(persisted, content)), content),
      migrated: true,
    };
  }
